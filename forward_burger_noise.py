...
filename = 'Burger_noise_' + str(noise_level) + 'dt_train-test_' + str(dt) + '-' + str(dt_test) + '_seq_n_mc_' + str(n_seq_mc) +'_forward_mc_train_d' + str(num_train) + '_alpha_' + str(mc_alpha) + '_lr_' + str(learning_rate) + '_batch_' + str(batch_size) + '_nseq_' + str(n_seq) + '_layer_' + str(layers) + 'neurons' + str(units) + '_epochs_' + str(num_epochs)

wandb.init(project="2022_Summer_IJCFD-Special-Issue",
           entity="ucla", name=filename)
...
Train_data = pd.read_csv('data/U_burger2d_train_data_noise_' + str(noise_level) + '_d_' + str(num_train) + '_Nt_' + str(nt_train_data) + '.csv')
...
Test_data = pd.read_csv('data/U_burger2d_test_data' + str(num_test) + '_Nt_' + str(nt_test_data) + '_dt_test_' + str(dt_test) + '.csv')
...
def single_solve_forward(un):
    ...
    u = un + (- dt / dx * jnp.multiply(un, (jnp.roll(un, -1, axis=1) - jnp.roll(un, 1, axis=1))) / 2 
