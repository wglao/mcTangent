...
filename = 'wave1d_noise_' + str(noise_level) + 'dt_train-test_' + str(dt) + '-' + str(dt_test) + '_seq_n_mc_' + str(n_seq_mc) +'_backward_mc_train_d' + str(num_train) + '_alpha_' + str(mc_alpha) + '_lr_' + str(learning_rate) + '_batch_' + str(batch_size) + '_nseq_' + str(n_seq) + '_layer_' + str(layers) + 'neurons' + str(units) + '_epochs_' + str(num_epochs)
...
Train_data = pd.read_csv('data/U_wave1d_train_data_noise_' + str(noise_level) + '_d_' + str(num_train) + '_Nt_' + str(nt_train_data) + '.csv')
...
# Use Forward Euler to warm start Backward Euler (5 steps)

def single_solve_backward(un):
    u0 = un + (- dt / dx * (jnp.roll(un, -1) - jnp.roll(un, 1)) / 2 )
    u1 = un + (- dt / dx * (jnp.roll(u0, -1) - jnp.roll(u0, 1)) / 2 )
    u2 = un + (- dt / dx * (jnp.roll(u1, -1) - jnp.roll(u1, 1)) / 2 )
    u3 = un + (- dt / dx * (jnp.roll(u2, -1) - jnp.roll(u2, 1)) / 2 )
    u4 = un + (- dt / dx * (jnp.roll(u3, -1) - jnp.roll(u3, 1)) / 2 )
    u5 = un + (- dt / dx * (jnp.roll(u4, -1) - jnp.roll(u4, 1)) / 2 )
    return u5
...
def squential_ml_second_phase(i, args):
    ...
    # The backward model-constrained loss
